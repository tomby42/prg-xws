/* Ukázka kreslení do pixmapy a kopírování pixmapy do okna */
...
Colormap cmap; /* Standardní paleta */
...
Pixmap source; /* Obrázek, který se bude kopírovat do okna */
unsigned int src_width, src_height; /* Rozmìry pixmapy source */
...
XColor blue, yellow, gray; /* Barvy */
Pixmap tile, stipple; /* Vyplòovací pixmapy */
...
static void create_resources(int argc, char *argv[])
{
    ...
    /* Alokovat barvy */
    if(!XAllocNamedColor(display, cmap, "blue", &color, &blue) ||
       !XAllocNamedColor(display, cmap, "yellow", &color, &yellow) ||
       !XAllocNamedColor(display, cmap, "gray", &color, &gray)) {
	fprintf(stderr, "Cannot allocate color\n");
	exit(1);
    }
    
    /* Pixmapy pro vyplòování */
    XGetWindowAttributes(display, topwin, &attr);
    tile = XCreatePixmapFromBitmapData(display, topwin, pattern2_bits,
				       pattern2_width, pattern2_height,
				       yellow.pixel, blue.pixel,
				       (unsigned)attr.depth);
    stipple = XCreateBitmapFromData(display, topwin, pattern2_bits,
				    pattern2_width, pattern2_height);
   
    /* Vytvoøit GC */
    gc_values.background = gray.pixel;
    ...
    XSetTile(display, my_gc, tile);
    XSetStipple(display, my_gc, stipple);
    
    /* Vytvoøit pixmapu a nakreslit do ní obsah, který se pak bude kopírovat
     * do okna */
    src_width = topwin_w;
    src_height = topwin_h;
    source = XCreatePixmap(display, topwin, src_width, src_height,
			   (unsigned)attr.depth);
    /* Server pixmapu naplní náhodnými daty, proto ji nejdøív vyma¾eme */
    XSetForeground(display, my_gc, white);
    XFillRectangle(display, source, my_gc, 0, 0, src_width, src_height);
    XSetForeground(display, my_gc, black);
    XDrawRectangle(display, source, my_gc, 0, 0, src_width - 1, src_height - 1);
    XSetFillStyle(display, my_gc, FillSolid);
    XFillArc(display, source, my_gc, 10, 30, (topwin_w / 2) - 20,
	     (topwin_h / 2) - 40, 0, 360 * 64);
    XDrawString(display, source, def_gc, 10, 20, "FillSolid", 9);
   
    XSetFillStyle(display, my_gc, FillTiled);
    XFillArc(display, source, my_gc, 10 + ((int)topwin_w / 2), 30,
	     (topwin_w / 2) - 20, (topwin_h / 2) - 40, 0, 360 * 64);
    XDrawString(display, source, def_gc, 10 + ((int)topwin_w / 2), 20,
		"FillTiled", 9);
    
    XSetFillStyle(display, my_gc, FillStippled);
    XFillArc(display, source, my_gc, 10, 30 + ((int)topwin_h / 2),
	     (topwin_w / 2) - 20, (topwin_h / 2) - 40, 0, 360 * 64);
    XDrawString(display, source, def_gc, 10, 20 + ((int)topwin_h / 2),
		"FillStippled", 12);
    
    XSetFillStyle(display, my_gc, FillOpaqueStippled);
    XFillArc(display, source, my_gc, 10 + ((int)topwin_w / 2),
	     30 + ((int)topwin_h / 2), (topwin_w / 2) - 20,
	     (topwin_h / 2) - 40, 0, 360 * 64);
    XDrawString(display, source, def_gc, 10 + ((int)topwin_w / 2),
		20 + ((int)topwin_h / 2), "FillOpaqueStippled", 18);
...
static void expose(XExposeEvent *event)
{
    ...
    XCopyArea(display, source, topwin, my_gc, 0, 0, src_width, src_height,
	      (int)(topwin_w - src_width) / 2,
	      (int)(topwin_h - src_height) / 2);
}

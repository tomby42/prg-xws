...
Bool keyboard_grab; /* Aktivní keyboard grab */
unsigned int pointer_grab; /* Aktivní grabs */
...
static void expose(XExposeEvent *event)
{
    ...
    if(keyboard_grab)
	XDrawString(display, topwin, my_gc, 10, 30,
		    "Keyboard grabbed", (int)strlen("Keyboard grabbed"));
    else
	XDrawString(display, topwin, my_gc, 10, 30,
		    "Keyboard not grabbed",
		    (int)strlen("Keyboard not grabbed"));
    if(pointer_grab)
	XDrawString(display, topwin, my_gc, 10, 50,
		    "Pointer grabbed", (int)strlen("Pointer grabbed"));
    else
	XDrawString(display, topwin, my_gc, 10, 50,
		    "Pointer not grabbed", (int)strlen("Pointer not grabbed"));
}
...
static void event_loop(void)
{
	...
	switch(event.type) {
		...
		/* Nastavit grab my¹i */
		if(!pointer_grab) {
		    if(XGrabPointer(display, topwin, False, ButtonPressMask,
				    GrabModeAsync, GrabModeAsync,
				    event.xbutton.button == 1 ? topwin : None,
				    None, CurrentTime) == GrabSuccess) {
			pointer_grab = event.xbutton.button;
			debug("Pointer successfully grabbed\n");
		    } else
			debug("Pointer grab failed\n");
		} else {
		    if(event.xbutton.button == pointer_grab) {
			pointer_grab = 0;
			XUngrabPointer(display, CurrentTime);
		    }
		}
		/* Pøekreslit celé okno */
		XClearArea(display, topwin, 0, 0, 0, 0, True);
		break;
	    case KeyPress:
		XLookupString(&event.xkey, buf, 10, &keysym, NULL);
		debug("Key \"%s\" (string \"%s\") pressed\n",
		      XKeysymToString(keysym), buf);
		/* Nastavit grab klávesnice */
		if(!keyboard_grab) {
		    keygrab = keysym;
		    if(XGrabKeyboard(display, topwin, False, GrabModeAsync,
				     GrabModeAsync, 
				     CurrentTime) == GrabSuccess) {
			keyboard_grab = True;
			debug("Keyboard successfully grabbed\n");
		    } else
